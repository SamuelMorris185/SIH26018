...
async def run_demo():
    ...
    # Use the migrated application database; never create tables in a demo script.
    demo_engine = create_async_engine(settings.DATABASE_URL, echo=False)
    ...
    try:
        async with session_factory() as session:
            ...
            operator = await user_service.get_by_email(session, "operator_demo@revenue.gov.in") or await user_service.create_user(
                session,
                UserCreate(
            ...
            reviewer = await user_service.get_by_email(session, "reviewer_demo@revenue.gov.in") or await user_service.create_user(
                session,
                UserCreate(
